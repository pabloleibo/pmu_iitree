...
# Ahora (Límite de ej. 5000 tramas. Si se llena, ocupa RAM fija y no crece más)
...
# --- AJUSTES DE VENTANA DE TIEMPO ---
# ¿Cuánto tiempo máximo aceptamos de retraso? (Ej. 3 segundos)
MAX_DELAY_SECONDS = 3.0
ultima_config_ts = 0.0  # Almacena el timestamp de la última config recibida válida
...
def sanitize_float(value):
    ...
    if math.isinf(value) or math.isnan(value):
        return None
    return float(value) # <--- Ya no usamos Decimal, usamos float nativo
...
def enviar_batch_firehose(firehose_client, stream_name, items):
    """
    Toma un lote de datos (parametro items), los desagrega por PMU y los envía a Firehose
    en un solo paquete comprimido (Estrategia de Costo Optimo).
    """
    if not firehose_client or not stream_name:
        return

    try:
        lista_registros_planos = []

        for item in items:
            # Datos base (Tiempo, PDC)
            ts_utc = item['TimestampUTC']
            ts_epoch = item['Timestamp']  # FLOAT (Agregamos esto)
            pdc = item['PDC_ID']
            
            # Obtenemos el diccionario decodificado
            decoded = item.get('DecodedData', {})
            
            # DESAGREGACIÓN: Convertimos la jerarquía en lista plana
            for pmu_id_str, mediciones in decoded.items():
                registro = {
                    "ts_str": ts_utc,       # String ISO8601 (Lectura humana)
                    "ts_epoch": ts_epoch,   # Float (Ordenamiento rápido)
                    "pdc_id": int(pdc),
                    "pmu_id": int(pmu_id_str), # ID fundamental para filtrar en Athena
                    "freq": mediciones.get('frequency'),
                    "rocof": mediciones.get('rocof'),
                    "phasors": mediciones.get('phasors')
                }
                lista_registros_planos.append(registro)

        # EMPAQUETADO: Enviamos 1 solo registro JSON con N mediciones adentro
        if lista_registros_planos:
            paquete = {
                "tipo": "batch_pmu_flat",
                "data": lista_registros_planos
            }
            
            # Al usar float nativo, json.dumps funciona directo
            json_str = json.dumps(paquete) + '\n'
        ...
        print(f"ERROR FIREHOSE: {e}")

def guardar_en_kinesis_firehose_worker(region, stream_name):
    """
    Este hilo toma datos CRUDOS de la cola, los DECODIFICA, y genera
    la lista de datos para guardarlos en firehose
    """
    # Conexión a Firehose
    firehose = None
    try:
        if stream_name:
            ...
            print(f"INFO: Worker Kinesis iniciado. Destino: {stream_name}")
        else:
            print("ADVERTENCIA: No se configuró FIREHOSE_STREAM_NAME. Los datos se perderán.")
            return
    except Exception as e:
        print(f"ERROR FATAL FIREHOSE: {e}")
        return
    ...
    while True:
        try:
            # Bloqueante con timeout para permitir flushear el batch si deja de llegar data
            try:
                item_crudo = db_queue.get(timeout=1)
            except queue.Empty:
                # Si no hay datos nuevos, procesamos lo que haya quedado en el buffer
                if batch_buffer:
                    enviar_batch_firehose(firehose, stream_name, batch_buffer)
                    batch_buffer = []
                continue
            ...
            item_final = {
                ...
                'Timestamp': item_crudo['Timestamp'],
                'TimestampUTC': item_crudo['TimestampUTC'],
                'DecodedData': datos_decodificados
            }

            batch_buffer.append(item_final)
            db_queue.task_done()

            # Si juntamos 25 items (o los que quieras), enviamos en lote
            if len(batch_buffer) >= 25:
                enviar_batch_firehose(firehose, stream_name, batch_buffer)
                batch_buffer = []

        except Exception as e:
            print(f"ERROR HILO Kinesis: Fallo al procesar o guardar en Firehose: {e}")
            # Importante limpiar buffer si falla para no trabar
            batch_buffer = []
...
def decodificar_datos(data):
    ...
    fracsec = data[11] << 16 | data[12] << 8 | data[13]
    tiempo = soc + fracsec / timebase

    # Obtener tiempo actual del sistema (UTC) para comparar
    tiempo_actual = time.time() # Asegúrate de importar time
    # --- FILTRO 1: DESCARTAR DATOS MUY VIEJOS
    # Si el paquete tiene más de 3 segundos de antigüedad, lo ignoramos.
    # Esto elimina el problema de los datos de hace 12 segundos.
    if (tiempo_actual - tiempo) > MAX_DELAY_SECONDS:
        print(f"Dato descartado por viejo: {tiempo}")
        return
    
    # --- FILTRO 2: DESCARTAR FUTURO (Reloj desajustado) ---
    if (tiempo - tiempo_actual) > 1.0:
        return

    # obtengo cantidad de datos utiles
    largo_datos_utiles = (data[2] << 8 | data[3])

    # Caso 1: El tiempo ya existe en nuestro buffer
    if tiempo in datos:
        # VERIFICACIÓN CLAVE: ¿Ya tengo datos para esta PMU en este tiempo?
        if datos[tiempo][idx] is not None:
            # Si no es None, significa que ya procesé este paquete antes (Duplicado)
            # Simplemente retorno y no hago nada.
            # print(f"Duplicado detectado y descartado: PMU {idcode} en T={tiempo}")
            return 
        
        # Si es None, es el primer dato legítimo que llega
        datos[tiempo][idx] = data[14:largo_datos_utiles - 2]
    # Caso 2: Es un tiempo nuevo
    else:
        datos[tiempo] = [None] * NUM_FUENTES
        datos[tiempo][idx] = data[14:largo_datos_utiles - 2]
        bisect.insort(tiempos, tiempo)#tiempos.append(tiempo)
...
def decodificar_config(data):
    ...
    # 2. Extraer Timestamp de la trama de configuración PRIMERO
    # Bytes 6-9: SOC, Bytes 10-13: FRACSEC
    soc = data[6] << 24 | data[7] << 16 | data[8] << 8 | data[9]
    fracsec = data[11] << 16 | data[12] << 8 | data[13]
    # Limpia los bits de calidad del tiempo si los hubiera (el byte 10 a veces tiene flags)
    # Usamos timebase global (16777215)
    timestamp_recibido = soc + (fracsec / timebase)

    # 3. Verificar si es más nuevo que lo que tenemos guardado para ESTA PMU
    ultimo_ts_conocido = config_timestamps.get(idcode, 0.0)

    # Si el timestamp recibido es MENOR o IGUAL al que ya tengo, descarto.
    # (A menos que sea 0, que significa reinicio del concentrador)
    if timestamp_recibido <= ultimo_ts_conocido and ultimo_ts_conocido != 0.0:
        # Es una config vieja o duplicada
        return False

    # --- SI LLEGAMOS AQUI, ES UNA CONFIGURACIÓN NUEVA ---
    #print(f"-> Nueva Configuración detectada para PMU {idcode} (TS: {timestamp_recibido})")
    # Guardar el nuevo timestamp
    config_timestamps[idcode] = timestamp_recibido
...
def emitir_datos():
    ...
    #db_queue.put(item)
    try:
        # put_nowait: Intenta ponerlo. Si está llena, lanza error Full inmediatamente.
        db_queue.put_nowait(item)
    except queue.Full:
        print("ADVERTENCIA: La cola de DynamoDB está llena. Descartando trama para proteger RAM.")
        # Opcional: Podrías monitorear esto para saber si tu internet es muy lento
...
FIREHOSE_STREAM_NAME = config_nube.get('FIREHOSE_STREAM_NAME', None) 

# --- Crear e iniciar el hilo para la base de datos ---
db_thread = threading.Thread(
    target=guardar_en_kinesis_firehose_worker,
    args=(AWS_REGION, FIREHOSE_STREAM_NAME),
    daemon=True
)
db_thread.start()
...
try:
    ...
    # --- CONFIGURACIÓN DE SEGURIDAD (La magia) ---
    context = ssl.create_default_context()
    context.load_verify_locations(CA_ROOT_CERT_FILE)
    context.load_cert_chain(certfile=THING_CERT_FILE, keyfile=THING_PRIVATE_KEY)

    # IMPORTANTE: Desactivar chequeo de hostname (por la diferencia IP interna/externa)
    context.check_hostname = False 
    context.verify_mode = ssl.CERT_REQUIRED # Seguimos requiriendo que el certificado sea válido criptográficamente

    # 4. Asignar el contexto al cliente MQTT
...
while True:
    # ---------------------------------------------------------
    # PARTE A: MANTENIMIENTO MQTT (Configs y Estado)
    # ---------------------------------------------------------
    try:
        #time.sleep(1)
        if(fallo_mqtt == 1):
